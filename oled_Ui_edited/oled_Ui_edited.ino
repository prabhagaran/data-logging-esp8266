...
"https://script.google.com/macros/s/AKfycbwNf-883QptoCRoETpbnI65wq_dWME6lhcgabdhQNXSIPjxK59bhRrCIdNw3LdXh08/exec";

const char* DEVICE_ID = "INCUBATOR_01";
...
void loop() {
  ...
  if (millis() - lastHeaterUpdate > HEATER_INTERVAL) {
    lastHeaterUpdate = millis();
    updateHeaterControl();
    if (uiState == UI_HOME && millis() - lastUiActivity > 500) drawHome();
  }
  ...
  if (enc != 0) {
    if (uiState == UI_MENU) { menuIndex = (menuIndex + enc + 4) % 4; drawMenu(); }
    else if (uiState == UI_SETTINGS) { settingsIndex = (settingsIndex + enc + 4) % 4; drawSettings(); }
    else if (uiState == UI_SET_TEMP) { setTemp = constrain(setTemp + enc * 0.1, 30.0, 40.0); drawSetTemp(); }
    else if (uiState == UI_HEATER_MODE) { heaterModeIndex = (heaterModeIndex + enc + 2) % 2; drawHeaterMode(); }
    else if (uiState == UI_MANUAL_HEATER) {
      manualSelectIndex = (manualSelectIndex + enc + 2) % 2;
      drawManualHeater();
    }
    else if (uiState == UI_HYSTERESIS) { hysteresis = constrain(hysteresis + enc * 0.1, 0.1, 1.0); drawHysteresis(); }
  }

  if (button.isPressed()) {
    lastUiActivity = millis();

    if (uiState == UI_HOME) { uiState = UI_MENU; drawMenu(); }

    else if (uiState == UI_MENU) {
      if (menuIndex == 2) { settingsIndex = 0; uiState = UI_SETTINGS; drawSettings(); }
      else { uiState = UI_HOME; drawHome(); }
    }

    else if (uiState == UI_SETTINGS) {
      if (settingsIndex == 0) { uiState = UI_SET_TEMP; drawSetTemp(); }
      else if (settingsIndex == 1) { heaterModeIndex = heaterMode; uiState = UI_HEATER_MODE; drawHeaterMode(); }
      ...
      manualHeaterOn = (manualSelectIndex == 1);  // SAVE
      uiState = UI_SETTINGS;                      // EXIT
      drawSettings();
    }
  }
